        ...
        static void Main(string[] args)
        {
            ...
            foreach (var filedesc in descriptor.File)
                foreach (var service in filedesc.Service)
                {
                    ...
                    targetName = Path.Combine(outputDir, targetName + ".g.cs");
            ...
            List<NameInfo> names)
        {
            ...
            foreach (var method in service.Method)
            {
                ...
                writer.WriteLines(
                    ...
                    $"\t\t\t}};",
                    $"\t\t\tvar response = await ((srpc::IApiClientDefinition)this).PerformMessage?.Invoke(nm);",
            ...
            writer.WriteLines(
                $"\t\t\t\tdefault: throw new s::NotSupportedExpection($\"{{request.ApiFunction}} is not defined\");",
                $"\t\t\t}}",
                $"\t\t}}"
            );
            foreach (var method in service.Method)
